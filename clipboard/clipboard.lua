...
populateMenu = function(key)
   ...
   if (#clipboard_history == 0) then
      ...
      -- instert to the third entry since the first two is "Clear All" and "-"
      for k,v in pairs(clipboard_history) do
         if (string.len(v) > label_length) then
            table.insert(menuData,3, {title=subStringUTF8(v,0,label_length).."…", fn = function() putOnPaste(v,key) end }) -- Truncate long strings
         else
            table.insert(menuData,3, {title=v, fn = function() putOnPaste(v,key) end })
         end -- end if else
      end-- end for
   end-- end if else
   -- footer
